...
export function InviteLandingPage() {
  ...
  const acceptMutation = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("pages.inviteLanding.failedToAcceptInvite"));
    },
  });
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("pages.inviteLanding.inviteNotAvailable")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("pages.inviteLanding.inviteExpired")}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("pages.inviteLanding.firstAdminConfigured")}
          </p>
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("pages.inviteLanding.claimSecret")}</p>
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              ...
              {onboardingInstallPath && <p className="font-mono break-all">{t("pages.inviteLanding.installTo")} {onboardingInstallPath}</p>}
            </div>
          )}
          {(onboardingTextUrl || onboardingTextPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("pages.inviteLanding.agentOnboardingText")}</p>
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo"
            ? t("pages.inviteLanding.bootstrapPaperclipInstance")
            : companyName
              ? t("pages.inviteLanding.joinCompany", { company: companyName })
              : t("pages.inviteLanding.joinThisCompany")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {invite.inviteType !== "bootstrap_ceo" && companyName
            ? `${t("pages.inviteLanding.invitedToJoin", { company: companyName })} `
            : null}
          {t("pages.inviteLanding.inviteExpires", { date: dateTime(invite.expiresAt) })}.
        </p>
        ...
        {joinType === "agent" && invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-4 space-y-3">
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("pages.inviteLanding.agentName")}</span>
              <input
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("pages.inviteLanding.adapterType")}</span>
              <select
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                value={adapterType}
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {getAdapterLabel(type)}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${t("pages.inviteLanding.comingSoon")})` : ""}
                  </option>
                ))}
              </select>
            </label>
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("pages.inviteLanding.capabilitiesOptional")}</span>
              <textarea
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                rows={4}
                value={capabilities}
        ...
        {requiresAuthForHuman && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            {t("pages.inviteLanding.signInToSubmit")}
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>{t("pages.inviteLanding.signInCreateAccount")}</Link>
              </Button>
            </div>
          </div>
        )}
        ...
        >
          {acceptMutation.isPending
            ...
            : invite.inviteType === "bootstrap_ceo"
              ...
              : t("pages.inviteLanding.submitJoinRequest")}
        </Button>
      </div>
    </div>
  );
