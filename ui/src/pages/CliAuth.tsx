...
export function CliAuthPage() {
  ...
  if (sessionQuery.isLoading || challengeQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("pages.cliAuth.loadingChallenge")}</div>;
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("pages.cliAuth.challengeUnavailable")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {challengeQuery.error instanceof Error ? challengeQuery.error.message : t("pages.cliAuth.challengeInvalidOrExpired")}
          </p>
        </div>
      </div>
    );
  ...
  if (!challenge) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("pages.cliAuth.challengeUnavailable")}</div>;
  }
  ...
  if (challenge.status === "approved") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">{t("pages.cliAuth.cliAccessApproved")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("pages.cliAuth.cliCanFinish")}
          </p>
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">
            {challenge.status === "expired" ? t("pages.cliAuth.challengeExpired") : t("pages.cliAuth.challengeCancelled")}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("pages.cliAuth.startAgain")}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("pages.cliAuth.signInToApprove")}
          </p>
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(currentPath)}`}>{t("pages.cliAuth.signInCreateAccount")}</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("pages.cliAuth.approvePaperclipCli")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("pages.cliAuth.localProcessRequesting")}
        </p>
        ...
        {(approveMutation.error || cancelMutation.error) && (
          <p className="mt-4 text-sm text-destructive">
            {(approveMutation.error ?? cancelMutation.error) instanceof Error
              ...
              : t("pages.cliAuth.failedToUpdateChallenge")}
          </p>
        )}
        ...
        <div className="mt-5 flex gap-3">
          ...
          >
            {approveMutation.isPending ? t("pages.cliAuth.approving") : t("pages.cliAuth.approveCliAccess")}
          </Button>
          <Button
            type="button"
            variant="outline"
