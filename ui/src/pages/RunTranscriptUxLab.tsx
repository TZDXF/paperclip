      ...
      <div className="border-b border-border/60 bg-background/90 px-5 py-4">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline" className="uppercase tracking-[0.18em] text-[10px]">
            {t("pages.runTranscriptUxLab.runDetail")}
          </Badge>
        ...
        <div className="mt-2 text-sm font-medium">
          {t("pages.runTranscriptUxLab.transcript")} ({runTranscriptFixtureEntries.length})
        </div>
      </div>
    ...
    <div className="overflow-hidden rounded-xl border border-cyan-500/25 bg-background/85 shadow-[0_20px_50px_rgba(6,182,212,0.10)]">
      <div className="border-b border-border/60 bg-cyan-500/[0.05] px-5 py-4">
        <div className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-700 dark:text-cyan-300">
          {t("pages.runTranscriptUxLab.liveRuns")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("pages.runTranscriptUxLab.compactLiveTranscript")}
        </div>
      </div>
      <div className="px-5 py-4">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <span className="inline-flex items-center gap-1 rounded-full border border-border/70 bg-background/70 px-2.5 py-1 text-[11px] text-muted-foreground">
            {t("pages.runTranscriptUxLab.openRun")}
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-2 text-[11px] text-muted-foreground">
                {streaming ? t("pages.runTranscriptUxLab.liveNow") : t("pages.runTranscriptUxLab.finishedAgo", { time: "2m" })}
              </div>
            </div>
...
export function RunTranscriptUxLab() {
  ...
  const surfaceOptions = useMemo<Array<{
    id: SurfaceId;
    label: string;
    eyebrow: string;
    description: string;
    ...
    {
      id: "detail",
      label: t("pages.runTranscriptUxLab.runDetail"),
      eyebrow: t("pages.runTranscriptUxLab.fullTranscript"),
      description: t("pages.runTranscriptUxLab.longFormRunPage"),
      icon: MonitorCog,
    },
    {
      id: "live",
      label: t("pages.runTranscriptUxLab.issueWidget"),
      eyebrow: t("pages.runTranscriptUxLab.liveStream"),
      description: t("pages.runTranscriptUxLab.issueDetailLive"),
      icon: RadioTower,
    },
    {
      id: "dashboard",
      label: t("pages.runTranscriptUxLab.dashboardCard"),
      eyebrow: t("pages.runTranscriptUxLab.denseCard"),
      description: t("pages.runTranscriptUxLab.compactScanning"),
      icon: PanelsTopLeft,
    },
  ], [t]);
          ...
          <aside className="border-b border-border/60 bg-background/75 p-5 lg:border-b-0 lg:border-r">
            <div className="mb-5">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/25 bg-cyan-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-cyan-700 dark:text-cyan-300">
                ...
                {t("pages.runTranscriptUxLab.uxLab")}
              </div>
              <h1 className="mt-4 text-2xl font-semibold tracking-tight">{t("pages.runTranscriptUxLab.runTranscriptFixtures")}</h1>
              <p className="mt-2 text-sm text-muted-foreground">
                {t("pages.runTranscriptUxLab.builtFromRealRun")}
              </p>
            </div>
          ...
          <main className="min-w-0 p-5">
            <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em]">
                  {t("pages.runTranscriptUxLab.sourceRun")} {runTranscriptFixtureMeta.sourceRunId.slice(0, 8)}
                </Badge>
                <Badge variant="outline" className="rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em]">
            ...
            <div className="mb-5 flex flex-wrap items-center gap-2">
              <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("pages.runTranscriptUxLab.controls")}
              </span>
              ...
              >
                {streaming ? t("pages.runTranscriptUxLab.showSettledState") : t("pages.runTranscriptUxLab.showStreamingState")}
              </Button>
            </div>
