  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? `${t("pages.routineDetail.next")}: ${new Date(trigger.nextRunAt).toLocaleString()}`
            : trigger.kind === "webhook"
              ? t("pages.routineDetail.webhook")
              : t("pages.routineDetail.api")}
        </span>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("pages.routineDetail.label")}</Label>
          <Input
            value={draft.label}
        ...
        {trigger.kind === "schedule" && (
          <div className="md:col-span-2 space-y-1.5">
            <Label className="text-xs">{t("pages.routineDetail.schedule")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("pages.routineDetail.signingMode")}</Label>
              <Select
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("pages.routineDetail.replayWindowSeconds")}</Label>
              <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("pages.routineDetail.last")}: {trigger.lastResult}</span>}
        <div className="ml-auto flex items-center gap-2">
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("pages.routineDetail.rotateSecret")}
            </Button>
          )}
          <Button
            variant="outline"
          ...
          >
            ...
            {t("pages.routineDetail.saveTrigger")}
          </Button>
          <Button
            variant="ghost"
            size="sm"
...
export function RoutineDetail() {
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("routines.routines"), href: "/routines" }, { label: routine.title }]);
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({
        title: t("pages.routineDetail.failedToCopy", { label: label.toLowerCase() }),
        body: error instanceof Error ? error.message : t("errors.serverError"),
        tone: "error",
      });
    }
  };
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("pages.routineDetail.triggerAdded"),
        ...
        pushToast({
          title: t("pages.routineDetail.triggerAdded"),
          body: t("pages.routineDetail.triggerScheduleSaved"),
          tone: "success",
        });
      }
      await Promise.all([
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("pages.routineDetail.triggerSaved"),
        body: t("pages.routineDetail.cadenceSaved"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("pages.routineDetail.triggerDeleted"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
  ...
  const rotateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("pages.routineDetail.failedToRotateSecret"),
        body: error instanceof Error ? error.message : t("errors.serverError"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("pages.routineDetail.selectCompany")} />;
  }
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : t("pages.routineDetail.routineNotFound")}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived" ? t("pages.routineDetail.archived") : automationEnabled ? t("pages.routineDetail.enabled") : t("pages.routineDetail.paused");
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <textarea
          ...
          placeholder={t("pages.routineDetail.routineTitle")}
          rows={1}
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("pages.routineDetail.savedNowNotShownAgain")}</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookUrl} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("pages.routineDetail.url"), secretMessage.webhookUrl)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("pages.routineDetail.url")}
              </Button>
            </div>
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookSecret} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("pages.routineDetail.secret"), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("pages.routineDetail.secret")}
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          ...
          <InlineEntitySelector
            ...
            placeholder={t("pages.routineDetail.assignee")}
            noneLabel={t("pages.routineDetail.noAssignee")}
            searchPlaceholder={t("pages.routineDetail.searchAssignees")}
            emptyMessage={t("pages.routineDetail.noAssigneesFound")}
          ...
          <InlineEntitySelector
            ...
            placeholder={t("pages.routineDetail.project")}
            noneLabel={t("pages.routineDetail.noProject")}
            searchPlaceholder={t("pages.routineDetail.searchProjects")}
            emptyMessage={t("pages.routineDetail.noProjectsFound")}
      ...
      <MarkdownEditor
        ...
        placeholder={t("pages.routineDetail.addInstructions")}
        bordered={false}
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("pages.routineDetail.advancedDeliverySettings")}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("pages.routineDetail.concurrency")}</p>
              <Select
            ...
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("pages.routineDetail.catchUp")}</p>
              <Select
...
<TabsTrigger value="activity" className="gap-1.5">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("pages.routineDetail.addTrigger")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? t("pages.routineDetail.adding") : t("pages.routineDetail.addTrigger")}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("pages.routineDetail.noTriggersConfigured")}</p>
          ) : (
        ...
        <TabsContent value="runs" className="space-y-4">
          ...
          {(routineRuns ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("pages.routineDetail.noRunsYet")}</p>
          ) : (
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("pages.routineDetail.noActivityYet")}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
