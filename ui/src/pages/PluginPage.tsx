...
export function PluginPage() {
  ...
  useEffect(() => {
    if (pageSlot) {
      setBreadcrumbs([
        { label: t("pages.pluginPage.plugins"), href: "/instance/settings/plugins" },
  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("pages.pluginPage.selectCompany")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("pages.pluginPage.loading")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("pages.pluginPage.routeConflict", { route: pluginRoutePath })}
        </div>
      );
    }
  }
