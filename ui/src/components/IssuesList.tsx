...
const quickFilterPresets = (tFn: (key: string) => string) => [
  { label: tFn("common.all"), statuses: [] as string[] },
  { label: tFn("issues.markAsInProgress"), statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { label: tFn("issues.closed"), statuses: ["backlog"] },
  { label: tFn("issues.markAsDone"), statuses: ["done", "cancelled"] },
];
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? t("ui.me"))
  ...
  return (
    <div className="space-y-4">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? `${t("issues.filters")}: ${activeFilterCount}` : t("issues.filters")}</span>
                {activeFilterCount > 0 && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <span className="text-sm">{t("ui.noAssignee")}</span>
                        </label>
                        {currentUserId && (
                          <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            <Checkbox
                            ...
                            <span className="text-sm">{t("ui.me")}</span>
                          </label>
                        )}
                        {(agents ?? []).map((agent) => (
                    ...
                    {projects && projects.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("projects.project")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("ui.groupBy")}</span>
                </Button>
              </PopoverTrigger>
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issues.noIssuesMatchFilters")}
