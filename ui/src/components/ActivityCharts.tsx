...
export function RunActivityChart({ runs }: { runs: HeartbeatRun[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("ui:noRunsYet")}</p>;
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("ui:noIssues")}</p>;
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  const statusLabels: Record<string, string> = {
    todo: t("ui:statusLabels.todo", { defaultValue: "To Do" }),
    in_progress: t("ui:statusLabels.in_progress", { defaultValue: "In Progress" }),
    in_review: t("ui:statusLabels.in_review", { defaultValue: "In Review" }),
    done: t("ui:statusLabels.done", { defaultValue: "Done" }),
    blocked: t("ui:statusLabels.blocked", { defaultValue: "Blocked" }),
    cancelled: t("ui:statusLabels.cancelled", { defaultValue: "Cancelled" }),
    backlog: t("ui:statusLabels.backlog", { defaultValue: "Backlog" }),
  };
  const days = getLast14Days();
