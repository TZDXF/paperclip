...
const EXECUTION_WORKSPACE_MODES = (t: (key: string) => string) => [
  { value: "shared_workspace", label: t("ui.projectDefault") },
  { value: "isolated_workspace", label: t("ui.newIsolatedWorkspace") },
  { value: "reuse_existing", label: t("ui.reuseExistingWorkspace") },
] as const;
...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: t("ui.createdWithWarnings", { issueRef }),
          body: t("ui.filesNotAdded", { count: failures.length }),
          tone: "warn",
          action: prefix
            ? { label: t("ui.openIssue", { issueRef }), href: `/${prefix}/issues/${issueRef}` }
            : undefined,
        });
      }
      clearDraft();
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("ui.failedToCreateIssue");
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("issues.createIssue")}</span>
          </div>
          ...
          <div className="overflow-x-auto overscroll-x-contain">
            <div className="inline-flex items-center gap-2 text-sm text-muted-foreground flex-wrap sm:flex-nowrap sm:min-w-max">
              ...
              <InlineEntitySelector
                ...
                disablePortal
                noneLabel={t("ui.noAssignee")}
                searchPlaceholder={t("ui.searchAssignees")}
                emptyMessage={t("ui.noAssigneesFound")}
                onChange={(value) => {
              ...
              <InlineEntitySelector
                ...
                searchPlaceholder={t("ui.searchProjects")}
                emptyMessage={t("ui.noProjectsFound")}
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("projects.projectName")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
        ...
        {currentProject && currentProjectSupportsExecutionWorkspace && (
          <div className="px-4 py-3 shrink-0 space-y-2">
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("ui.executionWorkspace")}</div>
              <div className="text-[11px] text-muted-foreground">
                {t("ui.executionWorkspaceHelp")}
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && (
                ...
                >
                  <option value="">{t("ui.chooseExistingWorkspace")}</option>
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  Reusing {selectedReusableExecutionWorkspace.name} from {selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("ui.executionWorkspace")}.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="px-4 pb-2 shrink-0">
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("ui.model")}</div>
                  <InlineEntitySelector
                    value={assigneeModelOverride}
                    options={modelOverrideOptions}
                    placeholder={t("ui.default")}
                    disablePortal
                    noneLabel={t("ui.default")}
                    searchPlaceholder={t("ui.searchModels")}
                    emptyMessage={t("ui.noModelsFound")}
                    onChange={setAssigneeModelOverride}
                  />
                </div>
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("ui.thinkingEffort")}</div>
                ...
                {assigneeAdapterType === "claude_local" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">{t("ui.enableChrome")}</div>
                    <ToggleSwitch
        ...
        >
          ...
          >
            <MarkdownEditor
              ...
              placeholder={t("ui.addDescription")}
              bordered={false}
            ...
            <div className="mt-4 space-y-3 rounded-lg border border-border/70 p-3">
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("ui.documents")}</div>
                  <div className="space-y-2">
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("ui.attachments")}</div>
                  <div className="space-y-2">
          ...
          <div className="flex items-center gap-3">
            <div className="min-h-5 text-right">
              {createIssue.isPending ? (
                ...
                <span className="text-xs text-destructive">{t("ui.failedToCreateIssue")}</span>
              ) : null}
            </div>
            <Button
              size="sm"
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("ui.creating") : t("ui.createIssue")}</span>
              </span>
            </Button>
          </div>
        </div>
